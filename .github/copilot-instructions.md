...
- **主要文件**：
  - `app.py`：唯一主入口，包含所有数据模型、路由、业务逻辑、会话管理、自动登录、权限控制。
  - `static/`：静态资源（CSS、图片、上传文件）。
  - `templates/`：Jinja2 模板，页面结构与渲染逻辑。

## 数据模型与权限
- 用户分为普通用户和管理员（`is_admin` 字段）。
- 物品（Item）有四种状态：0-待审核，1-已通过，2-已拒绝，3-已找到。
...
## 本地开发与调试
- 数据库已固定为 SQLite（`lostfound.db`），无需配置环境变量。
- 启动命令：`python app.py`（如有 Flask CLI 支持可用 `flask run`）。
- 首次运行请访问 `/init-db` 路由自动初始化数据库和管理员。
- 静态资源样式集中于 `static/css/style.css`，页面风格现代简洁。

## 其他说明
- 未发现自动化测试、CI/CD 或特殊构建脚本。
- 所有模型和业务逻辑均在 `app.py`，如需扩展建议采用 Flask 蓝图或拆分模块。
